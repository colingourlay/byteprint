...
class TwitterBlueprint(Blueprint):
    ...
    fields = {
        ...
        'ul_id': forms.CharField(
            ...
            help_text = "Tweets are written into your HTML as an unordered \
                ...
                more than one widget on a page, each will need a unique id.",
    ...
    def render(self, widget_data):
        ...
        if widget_data['username']:
            ...
            output += "<ul id=\"" + ul_id + "\"><li>You need to have javascript enabled to view <a href=\"http://www.twitter.com/" + widget_data['username'] + "/\">@" + widget_data['username'] + "</a>'s Twitter status</li></ul>"
            ...
            output += "<script type=\"text/javascript\" src=\"http://twitter.com/statuses/user_timeline/" + widget_data['username'] + ".json?callback=twitterCallback2&count=" + count + "&&suppress_response_codes\"></script>"
        return output
